...
- 挨拶・雑談（「こんにちは」「おはよう」「こんばんは」「ありがとう」等）
  ※挨拶への応答は「ようこそ！」等の時間帯に依存しない中立的な表現を使用
...
A: 「ようこそ！本日はご訪問いただきありがとうございます。ShinAIについて、何かお知りになりたいことはございますか？」
...
※この場合は「お問い合わせ」案内は不要（情報収集段階のため）`;
    ...
    async getFallbackResponse(query) {
        ...
        // 時間帯に依存しない中立的な挨拶を使用
        if (q.match(/^(こんにちは|こんばんは|おはよう|はじめまして|よろしく|お願いします|やあ|hello|hi)/)) {
            return 'ようこそ！本日はご訪問いただきありがとうございます。ShinAIについて、何かお知りになりたいことはございますか？';
        }
