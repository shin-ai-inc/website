...
Q: 「あなたの名前を教えてください」
A: 「私はShinAIアシスタントです。AI導入のご相談から技術的な質問まで、お答えいたします」

Q: 「料金について教えて」
A: 「ShinAIの料金は、プロジェクトの規模やご要望に応じて柔軟に設定しております。まずはお話をお伺いする機会をいただけますと幸いです」
...
※この場合は「お問い合わせ」案内は不要（情報収集段階のため）`;
    ...
    async getFallbackResponse(query) {
        ...
        if (q.match(/(ありがとう|感謝|助かる|参考になる)/)) {
            return 'お役に立てて嬉しく存じます。他にもご質問がございましたら、お聞かせください';
        }
        ...
        if (relevantSections.length > 0) {
            ...
            let intro = '';
            if (q.includes('暗黙知')) {
                intro = 'ShinAIの暗黙知AI化サービスについてご案内いたします。\n\n';
            } else if (q.includes('業務効率') || q.includes('自動化') || q.includes('RPA')) {
                intro = '業務効率化AIサービスについてお答えいたします。\n\n';
            } else if (q.includes('料金') || q.includes('価格') || q.includes('費用')) {
                intro = '料金体系についてご説明いたします。\n\n';
            } else if (q.includes('導入') || q.includes('プロセス')) {
                intro = '導入プロセスについてご案内いたします。\n\n';
            } else {
                intro = `${section.title}に関する情報です。\n\n`;
            }

            return `${intro}${content}\n\nもっと詳しく知りたい点がございましたら、お聞かせください`;
        }

        // デフォルト応答（より自然に）
        return 'ご質問ありがとうございます。AIシステム開発、業務効率化、データ分析など、どのような点についてお知りになりたいですか。具体的にお聞かせいただければ、より詳しくご案内いたします';
    }
}
